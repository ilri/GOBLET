...
int main(int argc, char *argv[])
{
    ...
    TCLAP::SwitchArg clearSwitch("c","clear","Connect to remote host", cmd, false);
    cmd.add(databaseArg);
    cmd.add(shapeArg);
    cmd.add(pathArg);
    cmd.add(hostArg);
    ...
    bool clear = clearSwitch.getValue();
    ...
    else
    {
        ...
        if (!clear)
            sql = "UPDATE aggrtable SET classCode = null WHERE shapedataset = '" + tableName + "'";
        else
            sql = "DELETE FROM aggrtable WHERE shapedataset = '" + tableName + "'";

        if (!qry.exec(sql))
        {
            gbtLog(QObject::tr("Cannot reset aggregation table."));
