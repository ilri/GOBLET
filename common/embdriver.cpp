...
static QSqlError qMakeError(const QString& err, QSqlError::ErrorType type,
                            const GOBLETDriverPrivate* p)
{
    const char *cerr = p->mysql ? mysql_error(p->mysql) : 0;
    return QSqlError(QLatin1String("GOBLET: ") + err,
                            ...
                            MYSQL_STMT* stmt)
{
    ...
    return QSqlError(QLatin1String("GOBLET3: ") + err,
...
void GOBLETResultPrivate::bindBlobs()
{
    int i;
...
bool GOBLETResultPrivate::bindInValues()
{
    MYSQL_BIND *bind;
    char *field;
    int i = 0;
    ...
    while((fieldInfo = mysql_fetch_field(meta))) {
        ...
        if (qIsBlob(fieldInfo->type)) {
            ...
            // after mysql_stmt_exec() in GOBLETResult::exec()
            fieldInfo->length = 0;
            hasBlobs = true;
        } else {
...
GOBLETResult::GOBLETResult(const GOBLETDriver* db)
: QSqlResult(db)
{
    d = new GOBLETResultPrivate(db, this);
}

GOBLETResult::~GOBLETResult()
{
    cleanup();
    delete d;
}

QVariant GOBLETResult::handle() const
{
#if MYSQL_VERSION_ID >= 40108
    if(d->preparedQuery)
...
// if this isn't done subsequent queries will fail with "Commands out of sync"
    ...
    if (d->stmt) {
        if (mysql_stmt_close(d->stmt))
            printf("GOBLETResult::cleanup: unable to free statement handle");
        d->stmt = 0;
    }
...
bool GOBLETResult::fetch(int i)
{
    ...
    if (d->preparedQuery) {
        ...
        if (nRC) {
            ...
            if (nRC == 1)
#endif
                setLastError(qMakeStmtError(QCoreApplication::translate("GOBLETResult",
...
QVariant GOBLETResult::data(int field)
{

    if (!isSelect() || field >= d->fields.count()) {
        printf("GOBLETResult::data: column %d out of range", field);
        return QVariant();
    }
    ...
    int fieldLength = 0;
    const GOBLETResultPrivate::QMyField &f = d->fields.at(field);
    QString val;
    if (d->preparedQuery) {
        if (f.nullIndicator)
    ...
    printf("GOBLETResult::data: unknown data type");
    return QVariant();
}

bool GOBLETResult::isNull(int field)
{
   if (d->preparedQuery)
...
bool GOBLETResult::reset (const QString& query)
{
    ...
    if (mysql_real_query(d->driver->d->mysql, encQuery.data(), encQuery.length())) {
        setLastError(qMakeError(QCoreApplication::translate("GOBLETResult", "Unable to execute query"),
    ...
    if (!d->result && mysql_field_count(d->driver->d->mysql) > 0) {
        setLastError(qMakeError(QCoreApplication::translate("GOBLETResult", "Unable to store result"),
                    QSqlError::StatementError, d->driver->d));
        return false;
    }
...
int GOBLETResult::numRowsAffected()
{
    return d->rowsAffected;
}

QVariant GOBLETResult::lastInsertId() const
{
...
QSqlRecord GOBLETResult::record() const
{
    QSqlRecord info;
    MYSQL_RES *res;
...
bool GOBLETResult::nextResult()
{
    ...
    if (status > 0) {
        setLastError(qMakeError(QCoreApplication::translate("GOBLETResult", "Unable to execute next query"),
                     QSqlError::StatementError, d->driver->d));
        return false;
    } else if (status == -1) {
    ...
    if (!d->result && numFields > 0) {
        setLastError(qMakeError(QCoreApplication::translate("GOBLETResult", "Unable to store next result"),
                     QSqlError::StatementError, d->driver->d));
        return false;
    }
...
void GOBLETResult::virtual_hook(int id, void *data)
{
    switch (id) {
...
bool GOBLETResult::prepare(const QString& query)
{
    if(!d->driver)
        return false;
#if MYSQL_VERSION_ID >= 40108
    ...
    if (!d->stmt) {
        setLastError(qMakeError(QCoreApplication::translate("GOBLETResult", "Unable to prepare statement"),
                     QSqlError::StatementError, d->driver->d));
        return false;
    }
...
static int GOBLETConnectionCount = 0;
static bool GOBLETInitHandledByUser = false;

static void qLibraryInit()
{
#ifndef Q_NO_MYSQL_EMBEDDED
# if MYSQL_VERSION_ID >= 40000
    if (GOBLETInitHandledByUser || GOBLETConnectionCount > 1)
        return;
...
# endif
        printf("GOBLETDriver::qServerInit: unable to start server.");
    }
...
GOBLETDriver::GOBLETDriver(QObject * parent)
    : QSqlDriver(parent)
{
    init();
    qLibraryInit();
...
GOBLETDriver::GOBLETDriver(MYSQL * con, QObject * parent)
    : QSqlDriver(parent)
{
    init();
    if (con) {
        ...
        setOpen(true);
        setOpenError(false);
        if (GOBLETConnectionCount == 1)
            GOBLETInitHandledByUser = true;
    } else {
        qLibraryInit();
    }
}

void GOBLETDriver::init()
{
    d = new GOBLETDriverPrivate();
    d->mysql = 0;
    GOBLETConnectionCount++;
}

GOBLETDriver::~GOBLETDriver()
{
    GOBLETConnectionCount--;
    if (GOBLETConnectionCount == 0 && !GOBLETInitHandledByUser)
        qLibraryEnd();
    delete d;
}

bool GOBLETDriver::hasFeature(DriverFeature f) const
{
    switch (f) {
    case Transactions:
...
static void setOptionFlag(uint &optionFlags, const QString &opt)
{
    if (opt == QLatin1String("CLIENT_COMPRESS"))
        ...
        printf("GOBLETDriver::open: Unknown connect option '%s'", opt.toLocal8Bit().constData());
}

bool GOBLETDriver::open(const QString& db,
                         const QString& user,
    ...
    for (int i = 0; i < opts.count(); ++i) {
        ...
        if ((idx = tmp.indexOf(QLatin1Char('='))) != -1) {
            ...
            if (opt == QLatin1String("UNIX_SOCKET"))
                ...
                printf("GOBLETDriver::open: Illegal connect option value '%s'",
...
QSqlResult *GOBLETDriver::createResult() const
{
    return new GOBLETResult(this);
}

QStringList GOBLETDriver::tables(QSql::TableType type) const
{
    QStringList tl;
#if MYSQL_VERSION_ID >= 40100
...
QSqlIndex GOBLETDriver::primaryIndex(const QString& tablename) const
{
    QSqlIndex idx;
    if (!isOpen())
        return idx;
...
QSqlRecord GOBLETDriver::record(const QString& tablename) const
{
...
QVariant GOBLETDriver::handle() const
{
    return QVariant::fromValue(d->mysql);
}

bool GOBLETDriver::beginTransaction()
{
#ifndef CLIENT_TRANSACTIONS
    return false;
#endif
    if (!isOpen()) {
        printf("GOBLETDriver::beginTransaction: Database not open");
        return false;
    }
...
bool GOBLETDriver::commitTransaction()
{
#ifndef CLIENT_TRANSACTIONS
    return false;
#endif
    if (!isOpen()) {
        printf("GOBLETDriver::commitTransaction: Database not open");
        return false;
    }
...
bool GOBLETDriver::rollbackTransaction()
{
#ifndef CLIENT_TRANSACTIONS
    return false;
#endif
    if (!isOpen()) {
        printf("GOBLETDriver::rollbackTransaction: Database not open");
        return false;
    }
...
QString GOBLETDriver::formatValue(const QSqlField &field, bool trimStrings) const
{
    QString r;
    if (field.isNull()) {
        ...
        case QVariant::ByteArray:
            if (isOpen()) {
                ...
                printf("GOBLETDriver::formatValue: Database not open");
            }
            // fall through
        default:
            r = QSqlDriver::formatValue(field, trimStrings);
...
QString GOBLETDriver::escapeIdentifier(const QString &identifier, IdentifierType) const
{
...
bool GOBLETDriver::isIdentifierEscapedImplementation(const QString &identifier, IdentifierType type) const
{
    Q_UNUSED(type);
